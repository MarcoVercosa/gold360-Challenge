...
import { IRequestCancelActiveRegisterRepository, IParams, IReturnCheckIsActive } from '../../entities/requestCancelActiveRoute/IRequestCancelActiveRegisterRepository';
...
export class RequestCancelActiveRegisterRepository implements IRequestCancelActiveRegisterRepository {
    ...
    async UserIsAdminConfirm({ idToken, fullNameToken }: { idToken: string, fullNameToken: string }): Promise<boolean> {
        let isAdmin: any = await UserIsAdminConfirmRepository({ idToken, fullNameToken })
        return isAdmin
    }

    async RequestCancelActiveRegisterRepository({ fullName, email, active }: IParams) {
