...
async function ConnectAMQPQueueServe() {
    let nameServer = `amqp://${process.env.CREDENTIALS_CANCEL_USER_CONSUMER}:${process.env.CREDENTIALS_CANCEL_PASS_CONSUMER}@172.20.0.3:5672`;
...
async function ConnectCancelDeadQueue() {
    ...
    let nameServer = `amqp://${process.env.CREDENTIALS_DEAD_QUEUE_USER}:${process.env.CREDENTIALS_DEAD_QUEUE_PASS}@172.20.0.3:5672`;
    try {
        const connection = await (0, amqplib_1.connect)(nameServer);
